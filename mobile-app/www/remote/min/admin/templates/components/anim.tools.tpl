<div class='fixed top-0 left-0 full-x' id='admin-anim-tools'>
    ...
    <div class='relative full-x bg-slate' ng-if='stage.animStats.startEvents.length'>
        <ul class='flex-center-vertical p15-grid full-x overflow-x no-scrollbar'>
            <li style='white-space:nowrap' ng-repeat='event in stage.animStats.startEvents'>
                <button class='height-36 txt-18 radius-2 normal block'>{{event.animationName}} </button>
            </li>
        </ul>
        <ul class='ugrid-5 flex-center-vertical p15-grid full-x overflow-x no-scrollbar' ng-if='stage.animationOptions.showTweener'>
            <li class='relative'>
                ...
                <input type='range' ng-mouseup="stage.player.globals.jump_only = false;" ng-model='stage.player.start_offset' ng-change='stage.player.jumpTo(stage.player.start_offset)' min="0" max="100"> </input>
            </li>
        </ul>
    </div>
</div>