					...
					<div ng-class="{'bg-crimson':!animation.env.active, 'bg-moxie':animation.env.active}"  class="state-timeline-dot actual width-10 height-10 absolute round absolute"  style="margin-left:15px;left:{{animation.env.xLeft}}%;" ng-repeat='animation in animations' id="animation-{{animation.env}}">
						<div class="state-timeline-info absolute z-index-99 radius-2" ng-class="{'bg-crimson':!animation.env.active, 'bg-moxie':animation.env.active}" style="padding:2px; bottom:15px; width:250px; height:380px" ng-hide='!animation.env.collapsed'>
							...
							<div class="bg-white radius-2 relative full-xy">
								<h1 class='bg-moxie ugrid-3 txt-1 p05xy'>
									<span class='flex-wrap-center block animation-{{animation.env.id}}' style='width:20%'>
										#{{animation.env.id}}
									</span>
									<span class='flex-wrap-center text-left weight-600' style='font-size:12px;width:30%;'>{{animation.name}}</span>
									<span class='ugrid-5' style='width:50%;'>
										...
										<a class='flex-center svg-white svg-stroke-8 svg-24 opacity-50' ng-click="animation.play()" ng-include="root.base_url + 'shared/templates/components/svg/main/play.html'"></a>
										<a class='state-timeline-info-expand flex-center svg-white svg-stroke-8 svg-24 opacity-50' ng-click='animation.pause()' ng-include="root.base_url + 'shared/templates/components/svg/main/pause.html'"></a>
								...
								<div class='bg-white p15-grid full-x'> </div>
								...
								<div class="txt-slate txt-14 flex-vertical-center bg-charcoal full-x absolute top-0 left-0" style="white-space:nowrap; height:calc(100% - 85px); margin-top:85px;"  ng-init='tabIndex = animation.env.tabIndex'>
										...
										<ul class='full-xy flex-vertical-center flex-wrap' style='align-content:stretch'>
											<li class='ugrid-2 full-x flex-vertical-center bg-slate-50p' style='align-self:stretch'>
												<div class='full-x p10x text-center'>
													...
													<span>
														{{animation.css[animation.prefix+'Duration']}}s
													</span>
												</div>
												<div class='full-x p10x ugrid-3'>
													...
													<input style='width:80%;' type='range' ng-mouseup="animation.set('Duration')" ng-model="animation.css[animation.prefix+'Duration']"/>
													</input>
													<span style='width:10%;'>
														10
													</span>
											...
											<li class='ugrid-2 full-x flex-vertical-center bg-charcoal p05y' style='align-self:stretch'>
												<div class='full-x p10x text-center'>
													...
													<span>
														{{animation.css[animation.prefix+'Delay']}}s
													</span>
												</div>
												<div class='full-x p10x ugrid-3'>
													<span style='width:10%;'>
													...
													<input style='width:80%;' type='range' ng-mouseup="animation.set('Delay')" ng-model="animation.css[animation.prefix+'Delay']"/>
													</input>
													<span style='width:10%;'>
														10
													</span>
											...
											<li class='ugrid-2 full-x bg-slate-50p p05y' style='align-self:stretch'>
												Iterations
												<br>
												<span class='p05xy'>
													<input style='width:80%;' type='range' ng-mouseup="animation.set('IterationCount')" ng-model="animation.css[animation.prefix+['IterationCount']]"/>
													</input>
												</span>
												<span>
													<input style='width:20%;' class='bg-smoke' type='checkbox' ng-model="animation.env.iterationChecked" ng-change="animation.set('IterationCount')"/>
													</input>
												</span>

											</li>
											<li class='ugrid-2 full-x bg-charcoal p05y flex-vertical-center txt-1' style='align-self:stretch; font-size:10px !important;'>
												<span class='flex-wrap p05x weight-700' style='width:20%'>
												Fill mode:
												</span>
												<div class='ugrid-4 flex-vertical-center text-center' style='width:80%;'>
													<span style='align-self:stretch' ng-class="{'opacity:50': option !== animation.css[prefix + 'fillMode']}" ng-repeat='option in animation.env.fillModeOptions' ng-click="animation.set('FillMode', options[$index])">
														{{option}}
													</div>
												</span>
											</li>
											<li class='ugrid-2 full-x bg-slate-50p p05y flex-vertical-center txt-1' style='align-self:stretch; font-size:10px !important;'>
												...
												<span class='ugrid-4 flex-vertical-center' style='width:80%;'>
													<span style='align-self:stretch' ng-class="{'opacity:50': option !== animation.css[prefix + 'fillMode']}" ng-repeat='option in animation.env.directionOptions' ng-click="animation.set('FillMode', options[$index])">
														{{option}}
													</span>
												</span>
											</li>
											<li class='ugrid-2 full-x flex-vertical-center bg-charcoal' style='align-self:stretch;'>
												...
												<span> {{animation.css[prefix + 'TimingFunction'] || 'ease'}} </span>
											</li>
											<li class='ugrid-2 full-x flex-vertical-center bg-slate-50' style='align-self:stretch;'>
												...
												<span class='ugrid-4 text-center flex-wrap-center' style='width:80%; font-size:8px;'>
													<span ng-if='animation.env.toggles.active'>Active</span>
													<span ng-if='!animation.env.toggles.active'>Inactive</span>
													<span>Collapse</span>
													<span>Hide</span>
													<span>Isolate</span>
												</span>
