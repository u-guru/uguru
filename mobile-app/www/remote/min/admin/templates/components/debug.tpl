<div>
    ...
    <div class='bg-auburn txt-white fixed bottom-0 left-0 full-x z-index-999' id='admin-anim-tools' ng-if='debug.options.showToolbar'>
        <div class="flex full-x">
            <div class="bg-moxie txt-white p15xy flex-center-vertical width-128">
            ...
            <ul class="relative flex-center-vertical-wrap p15-grid flex-stretch" style="width: calc(100% - 128px);">
                <li ng-repeat='state in debug.states' ng-click='playState(debug.states[$index])' ng-show="!debug.options.toggles.settings">
                    <button class="height-36 txt-18 radius-2 normal flex-center" ng-class="{'bg-smoke txt-moxie weight-600':state.played, 'bg-moxie':!state.played}">
                        <span>#{{state.id}} | {{state.trigger}}</span>
                        <span ng-if='state.played' class='svg-36-24 svg-stroke-7 stroke-moxie' ng-include="root.base_url + 'shared/templates/components/svg/main/check.html'"></span>
                    </button>
                </li>
