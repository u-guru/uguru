		...
		link: function(scope, element, attr) {
			$timeout(function() {
				scope.$watch(function() {
					...
					if (element.hasClass('show') || (attr.animOnShow && attr.animOnShow.length && element.hasClass(attr.animOnShow) && (attr.animOnShow === attr.animOnHide || !element.hasClass(attr.animOnHide)))) {
			...
			$timeout(function() {
				...
				if (elemHasManyWp.length > 1) {
					...
					for (var i = 0; i < elemHasManyWp.length; i++) {
						...
						scope.page.waypoints[indexWpName] = {
							offset: indexWpOffset || 0,
							activated: false,
							direction: null
						};
					...
					scope.page.waypoints[attr.initWp] = {
						offset: attr.wpOffset || 0,
						activated: false,
						direction: null
					};
        ...
        link: function(scope, element, attr) {
            ...
            element.bind('click', function() {
              if (!element[0].style.webkitTransform && !element[0].style.MozTransform && !element[0].style.msTransform && !element[0].style.OTransform && !element[0].style.transform) {
                ...
                var allTranslateOnClickElems = document.querySelectorAll("[translate-on-click]");
                for (var i = 0; i < allTranslateOnClickElems.length; i++) {
                  var indexTranslateElem  = allTranslateOnClickElems[i];
                  if (indexTranslateElem !== element[0] && indexTranslateElem.className.indexOf('translate-active') > -1) {
