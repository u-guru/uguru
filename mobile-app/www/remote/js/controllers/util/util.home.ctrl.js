...
.controller('HomeController', [
	...
	function($scope, $state, $stateParams, Restangular, User, $ionicSideMenuDelegate,
		...
		$scope.componentList = [{
			type: 'university',
			fields: ['name', 'num_popular_courses', 'start date', 'city', 'state', 'longitude', 'latitude', 'days til start', 'num_courses', 'school_color_one', 'school_color_two', 'banner_url', 'short_name', 'name', 'popular_courses']
		}];
		...
		$scope.toggleSidebar = function() {
			...
			if ($scope.page.sidebar.show) {
				$scope.links = [];
				$timeout(function() {
					$scope.links.push(pageLinks[0])
				}, 1000)
				$timeout(function() {
					$scope.links.push(pageLinks[1])
				}, 1000)
				$timeout(function() {
					$scope.links.push(pageLinks[2])
				}, 1000)
				$timeout(function() {
					$scope.links.push(pageLinks[3])
				}, 1000)
			} else {
				$scope.links = [];
			}
		...
		var showWindow = function(university) {
			...
			if ($scope.delayedUniversity && $scope.delayedUniversity.id === $scope.window.university.id) {
				...
				$scope.window.coords = {
					latitude: $scope.window.university.latitude,
					longitude: $scope.window.university.longitude
				};
		...
		var createRandomMarker = function(i, bounds, university, idKey) {
			...
			var ret = {
				...
				options: {
					...
					labelContent: "data:image/svg+xml,<svg viewBox='0 0 73 41' version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'><path d='M71.7272013,15.1343641 L63.071575,20.5 L72.2393802,25.9924931 L0,41 L1.42108547e-14,0 L71.7272013,15.1343641 L71.7272013,15.1343641 Z' id='flag' opacity='0.9' fill='" + university.school_color_dark + "'></path><path d='M0,0 L0,41 L6.261,39.7 L6.261,1.321 L0,0 Z' id='border' fill='" + university.school_color_light + "'></path><text font-family='Source Sans Pro' font-size='16' font-weight='600' line-spacing='16' fill='" + university.school_color_light + "'><tspan x='8' y='26'>" + universityName + "</tspan></text></svg>"
				},
