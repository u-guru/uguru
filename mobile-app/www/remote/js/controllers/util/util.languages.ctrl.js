...
.controller('LanguagesController', [
  ...
  function($scope, $state, $timeout, $localstorage, $ionicPlatform,
    ...
    $ionicSideMenuDelegate, Utilities) {
    ...
    $scope.static.languages = languages.slice(0, languages.length);
    ...
    $scope.addSelectedGuruLanguage = function(language, input_text, $index) {
      ...
      $scope.loader.showSuccess(language.name + ' successfully added', 2000);
    ...
    $scope.increaseLimit = function() {
      if($scope.static.languages && $scope.limit < $scope.static.languages.length) {
        $scope.limit += 10;
      }
    }
    ...
    $scope.query = function(input) {
      if (input.length === 0) {
        $scope.static.languages = languages;
      }
      $scope.static.languages = Utilities.nickMatcher(input, $scope.static.languages);
    }
