...
def add_students_to_mailing_list(university_name, student_objs):
    ...
    standard_keys = ['name', 'email', 'gender', 'year', 'type', 'phone', 'major','first_name']
    # validate dictionary
...
def get_all_university_progress():
    ...
    if results_arr:
        ...
        for result in results_arr:
            item = '#%d. %s has %d students' % (index, result['name'], result['count'])
            index+=1
            name_dict = {}
            name_dict['name'] =  result['name']
        
            # array.append(name_dict)
            with open('uc_name.json','wb') as outfile:
