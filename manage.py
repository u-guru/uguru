...
if arg == 'send_campaign_four':
    ...
    for key in batch_4_emails.keys():
        ...
        if not user:
            from app.emails import mailgun_campaign_four
            mailgun_campaign_four(receiver_name, receiver_email, 'chloe_3_100_text')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_five':
    from app.static.data.fa14_batch.batch_1 import batch_5_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_5_emails.keys():
        ...
        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_4_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_six':
    from app.static.data.fa14_batch.batch_1 import batch_6_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_6_emails.keys():
        ...
        if not user:
            from app.emails import mailgun_campaign_six
            mailgun_campaign_six(receiver_name, receiver_email, 'chloe_5_500_text')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_seven':
    from app.static.data.fa14_batch.batch_1 import batch_7_emails
    sent_count = 0
    avoided_count = 0
    index = 0
...
if arg == 'send_campaign_eight':
    from app.static.data.fa14_batch.batch_1 import batch_8_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_8_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_8_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()
...
if arg == 'send_campaign_nine':
    from app.static.data.fa14_batch.batch_1 import batch_9_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_9_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_9_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_8_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_ten':
    from app.static.data.fa14_batch.batch_1 import batch_10_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_10_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_10_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_9_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_eleven':
    from app.static.data.fa14_batch.batch_1 import batch_11_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_11_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_11_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_10_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_twelve':
    from app.static.data.fa14_batch.batch_1 import batch_12_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_12_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_12_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_11_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_thirteen':
    from app.static.data.fa14_batch.batch_1 import batch_13_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_13_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep 
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_13_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_12_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_fourteen':
    from app.static.data.fa14_batch.batch_1 import batch_14_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_14_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep 
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_14_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()

        if not user:
            from app.emails import mailgun_campaign_five
            mailgun_campaign_five(receiver_name, receiver_email, 'chloe_13_500_template')
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
        else:
            print receiver_name, receiver_email, 'already has an account'
...
if arg == 'send_campaign_twenty':
    from app.static.data.fa14_batch.batch_1 import batch_20_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    for key in batch_20_emails.keys():
        if index > 0  and index % 100 == 0:
            from time import sleep 
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_20_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()
...
if arg == 'send_campaign_twenty_one':
    from app.static.data.fa14_batch.batch_1 import batch_21_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep 
    ...
    for key in batch_21_emails.keys():
        if index > 0  and index % 100 == 0:
            ...
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_21_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()
...
if arg == 'send_campaign_twenty_two':
    from app.static.data.fa14_batch.batch_1 import batch_22_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep 
    sleep(1)
    for key in batch_22_emails.keys():
        if index > 0  and index % 100 == 0:
            print "100 emails sent, waiting 9 minutes"
            sleep(540)
        receiver_name = key.title()
        receiver_email = batch_22_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()
        ...
        if not user:
            ...
            import requests
            try:
                mailgun_campaign_five(receiver_name, receiver_email, 'chloe_22_200_template')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_one':
    from app.static.data.fa14_batch.batch_1 import batch_31_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep 
    for key in batch_31_emails.keys():
        if index > 0  and index % 25 == 0:
            from time import sleep 
            print "100 emails sent, waiting 9 minutes"
            sleep(120)
        receiver_name = key.title()
        receiver_email = batch_31_emails[key]
        from app.models import User
        user = User.query.filter_by(email=receiver_email).first()
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_jen_31_150')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_three': 
    ...
    index = 0
    from time import sleep 
    for key in batch_33_emails.keys():
        sleep(2)
...
if arg =='send_campaign_thirty_four':
    ...
    index = 0
    from time import sleep
    for key in batch_34_emails.keys():
        sleep(2)
        if index > 0  and index % 50 == 0:
            from time import sleep 
            print "50 emails sent, waiting 3 minutes"
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_lily_34_1700')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_five':
    from app.static.data.fa14_batch.batch_1 import batch_35_emails, batch_35_emails_used
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep
    for key in batch_35_emails.keys():
        sleep(2)
        if index > 0  and index % 50 == 0:
            from time import sleep 
            print "50 emails sent, waiting 3 minutes"
            sleep(180)
        receiver_name = key.title()
        receiver_email = batch_35_emails[key]

        if receiver_email in batch_35_emails_used:
            print "we have already emailed", receiver_email
            continue
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_lily_35_1000')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_six':
    from app.static.data.fa14_batch.batch_1 import batch_36_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep
    for key in batch_36_emails.keys():
        ...
        if index > 0  and index % 50 == 0:
            ...
            sleep(180)
        receiver_name = key.title()
        receiver_email = batch_36_emails[key]
...
if arg =='send_campaign_thirty_seven':
    from app.static.data.fa14_batch.batch_1 import batch_37_emails
    sent_count = 0
    avoided_count = 0
    index = 0
    from time import sleep
    for key in batch_37_emails.keys():
        sleep(2)
        if index > 0  and index % 50 == 0:
            from time import sleep 
            print "50 emails sent, waiting 3 minutes"
            sleep(180)
        receiver_name = key.title()
        receiver_email = batch_37_emails[key]
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_lily_37_1200')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_eight':
    ...
    for key in batch_38_emails.keys():
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_lily_38_2000')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_thirty_nine':
    ...
    index = 0
    from time import sleep
    for key in batch_39_emails.keys():
        receiver_name = key.title()
        ...
        if not user:
            ...
            import requests
            try:
                send_mandrill_nine(receiver_name, receiver_email, 'mandrill_fa_14_mass_lily_39_3000')
            except requests.exceptions.ConnectionError:
                continue
            print receiver_name, receiver_email, 'has been sent an email'
            sent_count += 1
...
if arg =='send_campaign_fourty':
    ...
    index = 0
    from time import sleep
    for key in batch_40_emails.keys():
        receiver_name = key.title()
            ...
            if r:
                ...
                for _r in r:
                    if _r.connected_tutor_id:
                        ...
                        skill = Skill.query.get(skill_id)
                        skill_name = short_variations_dict[skill.name]
                        send_mandrill_purchase_package_promotion(u.name, u.email, skill_name, tutor_name)
                        print u.name, "received an email for ", skill_name, tutor_name
                        count += 1
                        break
    print count, "emails sent"
