<div class="absolute top-0 left-0 full-xy flex-center perspective-container">
	<svg class="absolute top-0 left-0 full-xy" width="800px" height="600px" viewBox="0 0 800 600">
		<defs>
			<clipPath id="w1-clip">
				<rect x="329" y="306" width="38" height="60"></rect>
			</clipPath>
			<clipPath id="w2-clip">
				<rect x="382" y="306" width="38" height="60"></rect>
			</clipPath>
			<clipPath id="w3-clip">
				<rect x="435" y="306" width="38" height="60"></rect>
			</clipPath>
			<clipPath id="w4-clip">
				<rect x="382" y="407" width="38" height="60"></rect>
			</clipPath>
			<clipPath id="w5-clip">
				<rect x="435" y="407" width="38" height="60"></rect>
			</clipPath>
			<clipPath id="cat-clip">
				<rect class="cat-mask" x="405" y="313" width="62" height="53"></rect>
			</clipPath>
		</defs>
	</svg>
	<svg class="absolute top-0 left-0 full-xy" width="800px" height="600px" viewBox="0 0 800 600" style="background: #BBF2FE;"
		init-after="day-transition" on-init="a:[opacity:0:1:3000:easeInOutSine:0:1:f]:delay-2500">
		<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
			<g class="left-tree" init-after="leaving">
				<rect class="l-canopy" fill="#B4CB5D" x="221" y="364" width="43" height="86" rx="21.5"
					u init-with="p:[tro:50% 80%]"
					when-tree-shake="a:[rustle:300:linear:0:3:ar]"></rect>
				<path d="M241.5,499.514151 L241.5,397.485849 L241.5,499.514151 Z" class="l-tree-trunk-1" stroke="#625317" stroke-width="5"></path>
				<path d="M241.46694,432.899271 L232.618869,424.0512 L241.46694,432.899271 Z" class="l-tree-trunk-2" stroke="#625317" stroke-width="5"></path>
				<path d="M241.618869,412.899271 L250.46694,404.0512 L241.618869,412.899271 Z" class="l-tree-trunk-3" stroke="#625317" stroke-width="5"></path>
			</g>
			<g class="right-tree"
				init-after="leaving" on-init="s:[tree-shake:public]:delay-5700">
				<rect class="e-tree-canopy" fill="#0F7959" x="538" y="364" width="43" height="86" rx="21.5"
					u init-with="p:[tro:50% 80%]"
					when-tree-shake="a:[rustle:300:linear:0:3:a]"></rect>
				<polygon class="e-tree-trunk-1" stroke="#625317" stroke-width="5" points="559.5 499.514151 559.5 397.485849"></polygon>
				<polygon class="e-tree-trunk-2" stroke="#625317" stroke-width="5" points="559.53306 432.899271 568.381131 424.0512"></polygon>
				<polygon class="e-tree-trunk-3" stroke="#625317" stroke-width="5" points="559.381131 412.899271 550.53306 404.0512"></polygon>
			</g>
			<g class="left-bush">
				<path d="M298.79867,504 C297.256668,481.09864 278.191583,463 254.899335,463 C231.607087,463 212.542002,481.09864 211,504 L298.79867,504 L298.79867,504 Z" class="ever-bush-big" fill="#B6D6B8"></path>
				<path d="M202.404326,504 C201.76496,494.504314 193.859925,487 184.202163,487 C174.544402,487 166.639367,494.504314 166,504 L202.404326,504 L202.404326,504 Z" class="ever-bush-solo" fill="#B6D6B8"></path>
				<path d="M265.404326,504 C264.754498,493.945744 256.720115,486 246.904326,486 C237.088538,486 229.054155,493.945744 228.404326,504 L265.404326,504 L265.404326,504 Z" class="ever-bush-small" fill="#63AC69"></path>
				<path d="M321,504 C319.953054,487.801477 307.008771,475 291.194444,475 C275.380118,475 262.435835,487.801477 261.388889,504 L321,504 L321,504 Z" class="ever-bush-medium" fill="#63AC69"></path>
			</g>
			<g class="right-bush">
				<path d="M501.20133,504 C502.743332,481.09864 521.808417,463 545.100665,463 C568.392913,463 587.457998,481.09864 589,504 L501.20133,504 L501.20133,504 L501.20133,504 Z" class="lime-bush-big" fill="#C8D39F"></path>
				<path d="M597.595674,504 C598.23504,494.504314 606.140075,487 615.797837,487 C625.455598,487 633.360633,494.504314 634,504 L597.595674,504 L597.595674,504 L597.595674,504 Z" class="lime-bush-solo" fill="#C8D39F"></path>
				<path d="M529.595674,504 C530.245502,493.945744 538.279885,486 548.095674,486 C557.911462,486 565.945845,493.945744 566.595674,504 L529.595674,504 L529.595674,504 L529.595674,504 Z" class="lime-bush-small" fill="#93AA3C"></path>
				<path d="M474,504 C475.046946,487.801477 487.991229,475 503.805556,475 C519.619882,475 532.564165,487.801477 533.611111,504 L474,504 L474,504 L474,504 Z" class="lime-bush-medium" fill="#93AA3C"></path>
			</g>
			<g class="leaves"
				init-after="leaving" on-init="s:[leaf-fall:public:6500]">
				<g class="ref-leaf-a"
					u init-with="p:[op:0, transform:translateY(-10px)]"
					when-leaf-fall="a:[translateY:-10px:0px:300:easeInOutBack:0:1:f] | p:[op:1]">
					<path d="M606.083984,497.017578 L604.187448,500.796089 C603.127672,502.907502 600.448902,504.001567 598.229722,503.227902 C598.229722,503.227902 596.506449,502.689824 595.957136,502.343444 C595.011023,501.746855 592.317797,499.777724 592.317797,499.777724 L597.446445,495.770785 C599.308648,494.315872 602.355483,494.269454 604.257069,495.671034 L606.083984,497.017578 L606.083984,497.017578 Z" class="ref-leaf-a-1" fill="#107959"></path>
					<path d="M596.805664,499.794922 C596.805664,499.794922 601.556519,499.672017 604.170506,497.905234 C606.784493,496.13845 607.264987,493.117312 607.264987,493.117312" class="ref-leaf-a-2" stroke="#C8D39F" stroke-width="2"></path>
				</g>
				<g class="leaf-a"
					u init-with="p:[op:0, transform:translateY(-60px)]"
					when-leaf-fall="a:[translateY:-60px:0px:3000:easeInOutBack:600:1:f] | p:[op:1]:delay-600">
					<path d="M576.083984,416.017578 L574.187448,419.796089 C573.127672,421.907502 570.448902,423.001567 568.229722,422.227902 C568.229722,422.227902 566.506449,421.689824 565.957136,421.343444 C565.011023,420.746855 562.317797,418.777724 562.317797,418.777724 L567.446445,414.770785 C569.308648,413.315872 572.355483,413.269454 574.257069,414.671034 L576.083984,416.017578 L576.083984,416.017578 Z" class="leaf-a-1" fill="#107959"></path>
					<path d="M566.805664,418.794922 C566.805664,418.794922 571.556519,418.672017 574.170506,416.905234 C576.784493,415.13845 577.264987,412.117312 577.264987,412.117312" class="leaf-a-2" stroke="#C8D39F" stroke-width="2"></path>
				</g>
				<g class="ref-leaf-b"
					u init-with="p:[op:0, transform:translateY(-10px)]"
					when-leaf-fall="a:[translateY:-10px:0px:300:easeInOutBack:300:1:f] | p:[op:1]:delay-300">
					<path d="M509.916016,498.017578 L511.812552,501.796089 C512.872328,503.907502 515.551098,505.001567 517.770278,504.227902 C517.770278,504.227902 519.493551,503.689824 520.042864,503.343444 C520.988977,502.746855 523.682203,500.777724 523.682203,500.777724 L518.553555,496.770785 C516.691352,495.315872 513.644517,495.269454 511.742931,496.671034 L509.916016,498.017578 L509.916016,498.017578 Z" class="ref-leaf-b-1" fill="#107959"></path>
					<path d="M519.194336,500.794922 C519.194336,500.794922 514.443481,500.672017 511.829494,498.905234 C509.215507,497.13845 508.735013,494.117312 508.735013,494.117312" class="ref-leaf-b-2" stroke="#C8D39F" stroke-width="2"></path>
				</g>
				<g class="leaf-b"
					u init-with="p:[op:0, transform:translateY(-10px)]"
					when-leaf-fall="a:[translateY:-10px:0px:1300:easeInOutBack:0:1:f] | p:[op:1]">
					<path d="M539.916016,423.017578 L541.812552,426.796089 C542.872328,428.907502 545.551098,430.001567 547.770278,429.227902 C547.770278,429.227902 549.493551,428.689824 550.042864,428.343444 C550.988977,427.746855 553.682203,425.777724 553.682203,425.777724 L548.553555,421.770785 C546.691352,420.315872 543.644517,420.269454 541.742931,421.671034 L539.916016,423.017578 L539.916016,423.017578 Z" class="leaf-b-1" fill="#107959"></path>
					<path d="M549.194336,425.794922 C549.194336,425.794922 544.443481,425.672017 541.829494,423.905234 C539.215507,422.13845 538.735013,419.117312 538.735013,419.117312" class="leaf-b-2" stroke="#C8D39F" stroke-width="2"></path>
				</g>
				<g class="ref-leaf-d"
					u init-with="p:[op:0, transform:translateY(-30px) translateX(10px)]"
					when-leaf-fall="a:[translateY:-30px:0px:1300:easeInOutBack:300:1:f,translateX:10px:0px:1300:easeInOutBack:300:1:f] | p:[op:1]:delay-300">
					<path d="M191.916016,496.017578 L193.812552,499.796089 C194.872328,501.907502 197.551098,503.001567 199.770278,502.227902 C199.770278,502.227902 201.493551,501.689824 202.042864,501.343444 C202.988977,500.746855 205.682203,498.777724 205.682203,498.777724 L200.553555,494.770785 C198.691352,493.315872 195.644517,493.269454 193.742931,494.671034 L191.916016,496.017578 L191.916016,496.017578 L191.916016,496.017578 Z" class="ref-leaf-d-1" fill="#B4CB5D"></path>
					<path d="M201.194336,498.794922 C201.194336,498.794922 196.443481,498.672017 193.829494,496.905234 C191.215507,495.13845 190.735013,492.117312 190.735013,492.117312" class="ref-leaf-d-2" stroke="#63AC69" stroke-width="2"></path>
				</g>
				<g class="leaf-d"
					u init-with="p:[op:0, transform:translateY(0px), tro:center center]"
					when-leaf-fall="a:[translateY:0px:80px:2000:easeInOutExpo:400:1:f, rotate:0deg:15deg:200:easeInOutExpo:400:1:f] | p:[op:1]:delay-400">
					<path d="M221.916016,421.017578 L223.812552,424.796089 C224.872328,426.907502 227.551098,428.001567 229.770278,427.227902 C229.770278,427.227902 231.493551,426.689824 232.042864,426.343444 C232.988977,425.746855 235.682203,423.777724 235.682203,423.777724 L230.553555,419.770785 C228.691352,418.315872 225.644517,418.269454 223.742931,419.671034 L221.916016,421.017578 L221.916016,421.017578 L221.916016,421.017578 Z" class="leaf-d-1" fill="#B4CB5D"></path>
					<path d="M231.194336,423.794922 C231.194336,423.794922 226.443481,423.672017 223.829494,421.905234 C221.215507,420.13845 220.735013,417.117312 220.735013,417.117312" class="leaf-d-2" stroke="#63AC69" stroke-width="2"></path>
				</g>

				<g class="ref-leaf-c"
					u init-with="p:[op:0, transform:translateY(-10px)]"
					when-leaf-fall="a:[translateY:-10px:0px:2300:easeInOutBack:0:1:f] | p:[op:1]">
					<path d="M288.083984,498.017578 L286.187448,501.796089 C285.127672,503.907502 282.448902,505.001567 280.229722,504.227902 C280.229722,504.227902 278.506449,503.689824 277.957136,503.343444 C277.011023,502.746855 274.317797,500.777724 274.317797,500.777724 L279.446445,496.770785 C281.308648,495.315872 284.355483,495.269454 286.257069,496.671034 L288.083984,498.017578 L288.083984,498.017578 L288.083984,498.017578 Z" class="ref-leaf-c-1" fill="#B4CB5D"></path>
					<path d="M278.805664,500.794922 C278.805664,500.794922 283.556519,500.672017 286.170506,498.905234 C288.784493,497.13845 289.264987,494.117312 289.264987,494.117312" class="ref-leaf-c-2" stroke="#63AC69" stroke-width="2"></path>
				</g>
				<g class="leaf-c"
					u init-with="p:[op:0, transform:translateY(-10px)]"
					when-leaf-fall="a:[translateY:-10px:0px:1300:easeInOutBack:0:1:f] | p:[op:1]">
					<path d="M258.083984,428.017578 L256.187448,431.796089 C255.127672,433.907502 252.448902,435.001567 250.229722,434.227902 C250.229722,434.227902 248.506449,433.689824 247.957136,433.343444 C247.011023,432.746855 244.317797,430.777724 244.317797,430.777724 L249.446445,426.770785 C251.308648,425.315872 254.355483,425.269454 256.257069,426.671034 L258.083984,428.017578 L258.083984,428.017578 L258.083984,428.017578 Z" class="leaf-c-1" fill="#B4CB5D"></path>
					<path d="M248.805664,430.794922 C248.805664,430.794922 253.556519,430.672017 256.170506,428.905234 C258.784493,427.13845 259.264987,424.117312 259.264987,424.117312" class="leaf-c-2" stroke="#63AC69" stroke-width="2"></path>
				</g>
			</g>
			<g class="sky" init-after="day-transition" on-init="s:[day-sky:public:6000]">
				<circle class="sun" fill="#FECE67" cx="281" cy="171" r="44"
					u
					init-with="p:[transform:translateY(-800px)]"
					when-day-sky="a:[translateY:-800px:0px:400:easeInOutBack:0:1:f]:delay-4000 | s:[flying:public:5000]"></circle>
				<g 	u
					when-day-sky="a:[translateY:4px:-4px:1000:easeInOutQuad:0:i:ar]">
					<path d="M432.821041,232 C432.284244,229.754184 432,227.410286 432,225 C432,208.431458 445.431458,195 462,195 C478.568542,195 492,208.431458 492,225 C492,227.410286 491.715756,229.754184 491.178959,232 L432.821041,232 L432.821041,232 L432.821041,232 Z" class="day-b-cloud-1" fill="#FFFFFF"></path>
					<path d="M487.756105,232 C487.262518,230.08228 487,228.071797 487,226 C487,212.745166 497.745166,202 511,202 C524.254834,202 535,212.745166 535,226 C535,228.071797 534.737482,230.08228 534.243895,232 L487.756105,232 L487.756105,232 L487.756105,232 Z" class="day-b-cloud-2" fill="#FFFFFF"></path>
					<path d="M527.024234,232 C526.360925,230.123326 526,228.103813 526,226 C526,216.058875 534.058875,208 544,208 C553.941125,208 562,216.058875 562,226 C562,228.103813 561.639075,230.123326 560.975766,232 L527.024234,232 L527.024234,232 L527.024234,232 Z" class="day-b-cloud-3" fill="#FFFFFF"></path>
				</g>
				<g 	u
				 	when-day-sky="a:[translateY:4px:-4px:1000:easeInOutQuad:0:i:a]">
					<path d="M430.243895,178 C430.737482,176.08228 431,174.071797 431,172 C431,158.745166 420.254834,148 407,148 C393.745166,148 383,158.745166 383,172 C383,174.071797 383.262518,176.08228 383.756105,178 L430.243895,178 L430.243895,178 L430.243895,178 Z" class="day-s-cloud-1" fill="#FFFFFF"></path>
					<path d="M390.975766,178 C391.639075,176.123326 392,174.103813 392,172 C392,162.058875 383.941125,154 374,154 C364.058875,154 356,162.058875 356,172 C356,174.103813 356.360925,176.123326 357.024234,178 L390.975766,178 L390.975766,178 L390.975766,178 Z" class="day-s-cloud-2" fill="#FFFFFF"></path>
				</g>
				<g class="birds" init-after="flying" on-init="s:[fly-start:public, leaving:public:1100]">
					<g class="b-bird"
						u
						when-fly-start="a:[translateY:-3px:3px:1000:linear:0:i:a]">
						<path d="M499.5,126 C499.5,126 505.438375,126.265969 511.75,131.5 C518.061625,136.734031 520,143 520,143" class="b-bird-left" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:right bottom]"
							when-fly-start="a:[rotate:15deg:-10deg:1000:easeInOutCirc:0:i:a]"></path>
						<path d="M541,126 C541,126 535.061625,126.265969 528.75,131.5 C522.438375,136.734031 520.5,143 520.5,143" class="b-bird-right" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:left bottom]"
							when-fly-start="a:[rotate:-15deg:10deg:1000:easeInOutCirc:0:i:a]"></path>
					</g>
					<g class="m-bird" u
						when-fly-start="a:[translateY:-3px:3px:1000:linear:100:i:a]">
						<path d="M479.294118,158 C479.294118,158 483.136595,158.172098 487.220588,161.558824 C491.304581,164.945549 492.558824,169 492.558824,169" class="m-bird-left" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:right bottom]"
							when-fly-start="a:[rotate:10deg:-10deg:1000:easeInOutCirc:100:i:a]"></path>
						<path d="M506.558824,158 C506.558824,158 502.716346,158.172098 498.632353,161.558824 C494.54836,164.945549 493.294118,169 493.294118,169" class="m-bird-right" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:left bottom]"
							when-fly-start="a:[rotate:-10deg:10deg:1000:easeInOutCirc:100:i:a]"></path>
					</g>
					<g class="m-bird" u
						when-fly-start="a:[translateY:-3px:3px:1000:linear:150:i:a]">
						<path d="M344.294118,218 C344.294118,218 346.963583,218.062581 349.800835,219.294118 C352.638087,220.525654 353.509441,222 353.509441,222" class="far-bird-left" stroke="#6E6144" stroke-width="3"
							u
							init-with="p:[transform:rotate(0deg), tro:right bottom]"
							when-fly-start="a:[rotate:10deg:-10deg:1000:easeInOutCirc:150:i:a]"></path>
						<path d="M363.509441,218 C363.509441,218 360.839975,218.062581 358.002723,219.294118 C355.165471,220.525654 354.294118,222 354.294118,222" class="far-bird-right" stroke="#6E6144" stroke-width="3"
							u
							init-with="p:[transform:rotate(0deg), tro:left bottom]"
							when-fly-start="a:[rotate:-10deg:10deg:1000:easeInOutCirc:150:i:a]"></path>
					</g>
					<g u
						when-fly-start="a:[translateY:-3px:3px:1000:linear:175:i:a]">
						<path d="M512.294118,178 C512.294118,178 514.963583,178.062581 517.800835,179.294118 C520.638087,180.525654 521.509441,182 521.509441,182" class="small-bird-left" stroke="#6E6144" stroke-width="3"
							u
							init-with="p:[transform:rotate(0deg), tro:right bottom]"
							when-fly-start="a:[rotate:10deg:-10deg:1100:easeInOutCirc:175:i:a]"></path>
						<path d="M530.509441,178 C530.509441,178 527.839975,178.062581 525.002723,179.294118 C522.165471,180.525654 521.294118,182 521.294118,182" class="small-bird-right" stroke="#6E6144" stroke-width="3"
							u
							init-with="p:[transform:rotate(0deg), tro:left bottom]"
							when-fly-start="a:[rotate:-10deg:10deg:1100:easeInOutCirc:175:i:a]"></path>
					</g>
					<g u
						when-fly-start="a:[translateY:-3px:3px:1000:linear:0:i:a]">
						<path d="M308.509441,236 C308.284677,236 309.156031,234.525654 311.993282,233.294118 C314.830534,232.062581 317.5,232 317.5,232" class="stout-bird-right" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:left bottom]"
							when-fly-start="a:[rotate:-10deg:10deg:1000:easeInOutCirc:0:i:a]"></path>
						<path d="M308.466318,236 C308.691082,236 307.819728,234.525654 304.982476,233.294118 C302.145224,232.062581 299.475759,232 299.475759,232" class="stout-bird-left" stroke="#6E6144" stroke-width="4"
							u
							init-with="p:[transform:rotate(0deg), tro:right bottom]"
							when-fly-start="a:[rotate:10deg:-10deg:1000:easeInOutCirc:0:i:a]"></path>
					</g>
				</g>
			</g>
			<g class="house" init-after="day-transition" on-init="s:[blinds:public:7000]">
				<g>
					<rect class="house-foundation-beige" fill="#E5D8B7" x="307" y="285" width="187" height="204"></rect>
					<rect class="house-casing" fill="#FFFFFF" x="301" y="485" width="199" height="19"></rect>
					<rect class="roof-edge" fill="#FFFFFF" x="299" y="258" width="203" height="19"></rect>
					<rect class="roof-beige" fill="#E5D8B7" x="299" y="258" width="203" height="12"></rect>
					<rect class="roof-top" fill="#FFFFFF" x="296" y="258" width="209" height="7"></rect>
				</g>
				<g class="w5">
					<g clip-path="url(#w5-clip)">
						<rect class="w5-bg" fill="#BDE397" x="435" y="407" width="38" height="60"></rect>
						<rect class="w5-blind" fill="#A0857A" x="435" y="407" width="38" height="60"
							u init-with="p:[tro:center top]"
							when-blinds="a:[hiw-shade-bounceUp:2000:linear:0:i:a]"></rect>
					</g>
					<rect class="w5-shadow-btm" fill="#5B4F4B" x="434" y="477" width="40" height="5"></rect>
					<rect class="w5-ledge" fill="#C56C76" x="432" y="467" width="44" height="10"></rect>
					<rect class="w5-shadow-top" fill="#5B4F4B" x="440" y="414" width="28" height="7"></rect>
					<rect class="w5-shadow-mid" fill="#5B4F4B" x="440" y="443" width="28" height="7"></rect>
					<path d="M435,407 L473,407 L473,467 L435,467 L435,407 L435,407 L435,407 L435,407 Z M441,414 L467,414 L467,438 L441,438 L441,414 L441,414 L441,414 L441,414 Z M441,443 L467,443 L467,467 L441,467 L441,443 L441,443 L441,443 L441,443 Z" class="w5-casing" fill="#FFFFFF"></path>
				</g>
				<g class="w4">
					<g clip-path="url(#w4-clip)">
						<rect class="w4-bg" fill="#BDE397" x="382" y="407" width="38" height="60"></rect>
						<rect class="w4-blind" fill="#A0857A" x="382" y="407" width="38" height="60"></rect>
					</g>
					<rect class="w4-shadow-btm" fill="#5B4F4B" x="381" y="477" width="40" height="5"></rect>
					<rect class="w4-ledge" fill="#C56C76" x="379" y="467" width="44" height="10"></rect>
					<rect class="w4-shadow-top" fill="#5B4F4B" x="387" y="414" width="28" height="7"></rect>
					<g class="lamp">
						<path d="M405.612903,435.221429 L405.612903,465.778571" class="lamp-4" stroke="#77453B" stroke-width="2"></path>
						<path d="M399.032258,446.185714 L399.032258,458.528571" class="lamp-3" stroke="#C4A55A" stroke-dasharray="1,1"></path>
						<path d="M399.032258,461.228571 C399.673621,461.228571 400.193548,460.710501 400.193548,460.071429 C400.193548,459.432356 399.673621,458.142857 399.032258,458.142857 C398.390895,458.142857 397.870968,459.432356 397.870968,460.071429 C397.870968,460.710501 398.390895,461.228571 399.032258,461.228571 Z" class="lamp-2" fill="#F6C64E"></path>
						<polygon class="lamp-1" fill="#F7FFEB" opacity="0.2" points="398.722581 436.542857 413.277419 436.542857 417.404296 467.191407 390.701172 467.191407"></polygon>
						<path d="M398.208192,438.47543 C398.492281,437.408099 399.611141,436.542857 400.715851,436.542857 L411.284149,436.542857 C412.385002,436.542857 413.505199,437.398632 413.791808,438.47543 L417.485611,452.353142 C417.7697,453.420472 417.102974,454.285714 415.995053,454.285714 L396.004947,454.285714 C394.897645,454.285714 394.22778,453.429939 394.514389,452.353142 L398.208192,438.47543 Z" class="lamp-shade" fill="#F7FFEB"></path>
					</g>
					<rect class="w4-shadow-mid" fill="#5B4F4B" x="387" y="443" width="28" height="7"></rect>
					<path d="M382,407 L420,407 L420,467 L382,467 L382,407 L382,407 L382,407 L382,407 Z M388,414 L414,414 L414,438 L388,438 L388,414 L388,414 L388,414 L388,414 Z M388,443 L414,443 L414,467 L388,467 L388,443 L388,443 L388,443 L388,443 Z" class="w4-casing" fill="#FFFFFF"></path>
				</g>
				<g class="w3">
					<g clip-path="url(#w3-clip)">
						<rect class="w3-bg" fill="#BDE397" x="435" y="306" width="38" height="60"></rect>
						<rect class="w3-blind" fill="#A0857A" x="435" y="306" width="38" height="60"></rect>
					</g>
					<rect class="w3-shadow-btm" fill="#5B4F4B" x="434" y="376" width="40" height="5"></rect>
					<rect class="w3-ledge" fill="#C56C76" x="432" y="366" width="44" height="10"></rect>
					<rect class="w3-shadow-top" fill="#5B4F4B" x="440" y="313" width="28" height="7"></rect>
					<rect class="w3-shadow-mid" fill="#5B4F4B" x="440" y="342" width="28" height="7"></rect>
					<path d="M435,306 L473,306 L473,366 L435,366 L435,306 L435,306 L435,306 L435,306 Z M441,313 L467,313 L467,337 L441,337 L441,313 L441,313 L441,313 L441,313 Z M441,342 L467,342 L467,366 L441,366 L441,342 L441,342 L441,342 L441,342 Z" class="w3-casing" fill="#FFFFFF"></path>
					<g class="cat" clip-path="url(#cat-clip)">
						<path d="M474.696071,363.150827 C474.747194,363.171529 474.851114,363.21461 475.00015,363.278033 C475.252951,363.385613 475.535748,363.509545 475.840788,363.647739 C476.710637,364.041812 477.579533,364.468885 478.383191,364.910278 C479.4925,365.519542 480.3782,366.103814 480.932184,366.606317 C481.121684,366.778207 481.257239,366.927429 481.334268,367.039926 C481.35535,367.070715 481.345724,367.048013 481.334996,366.955797 C481.416925,367.660076 480.896727,367.944139 478.419194,368.080434 C476.027458,368.21201 474.900965,369.897935 475.092965,371.980774 C475.143943,372.533797 475.279042,373.086314 475.470836,373.634964 C475.611982,374.03873 475.754987,374.363177 475.863045,374.576361 C476.311218,375.460544 477.391304,375.814 478.275487,375.365827 C479.159669,374.917655 479.513125,373.837568 479.064953,372.953386 C479.023594,372.871792 478.942474,372.687748 478.859495,372.450377 C478.756796,372.156593 478.68832,371.876544 478.667553,371.651263 C478.653888,371.503021 478.661347,371.458751 478.619328,371.521637 C478.569601,371.59606 478.50081,371.656357 478.440842,371.686138 C478.444721,371.684212 478.498838,371.671224 478.616376,371.664758 C480.456482,371.563529 481.761925,371.316066 482.819282,370.738678 C484.358866,369.897962 485.118734,368.415318 484.900694,366.540998 C484.787397,365.567075 484.226604,364.748066 483.343964,363.94745 C482.54186,363.219885 481.445524,362.496661 480.111293,361.763863 C479.218548,361.273542 478.269868,360.807253 477.322142,360.3779 C476.99103,360.227894 476.683115,360.092954 476.40579,359.974938 C476.23675,359.903003 476.113474,359.851897 476.043425,359.823532 C475.124618,359.45147 474.078162,359.894695 473.7061,360.813503 C473.334039,361.73231 473.777264,362.778766 474.696071,363.150827 L474.696071,363.150827 Z" class="cat-1" fill="#D7A163"></path>
						<path d="M475.618603,369.844218 C476.767607,369.536343 477.53267,368.665794 477.327421,367.899791 C477.122171,367.133788 476.024331,366.762402 474.875327,367.070277 C473.726324,367.378152 472.96126,368.248701 473.166509,369.014704 C473.371759,369.780706 474.469599,370.152092 475.618603,369.844218 L475.618603,369.844218 Z" class="cat-2" fill="#FAA748"></path>
						<ellipse class="cat-3" fill="#FAA748" cx="467.769231" cy="363.641026" rx="10.7692308" ry="7.17948718"></ellipse>
						<path d="M468.271408,359.809467 C468.792383,359.030504 469.116668,358.105367 469.169274,357.101576 C469.324913,354.131811 467.043613,351.59817 464.073847,351.442531 C461.104082,351.286892 458.570441,353.568193 458.414802,356.537958 C458.259164,359.507724 459.950847,360.920829 459.950847,360.920829" class="cat-4" fill="#FAA748"></path>
						<path d="M457.921682,351.699497 C457.718178,350.935955 458.199967,350.380484 458.978566,350.504758 C458.978566,350.504758 459.580529,350.444919 460.135636,350.938198 C460.368268,351.144919 461.020676,351.549356 461.020676,351.549356 C461.853017,352.090124 461.830821,352.931307 460.971274,353.428089 L458.727959,354.72463 L457.921682,351.699497 L457.921682,351.699497 Z" class="cat-5" fill="#DFA461"></path>
						<path d="M458.162638,351.816307 C458.054347,351.259195 458.327711,351.086754 458.778106,351.43647 C458.778106,351.43647 459.4332,351.938403 459.619391,352.104066 C459.88244,352.338115 460.253848,352.664282 460.253848,352.664282 C460.678079,353.037744 460.632835,353.575294 460.141616,353.871679 L458.727959,354.72463 L458.162638,351.816307 L458.162638,351.816307 Z" class="cat-6" fill="#3D0044"></path>
						<path d="M469.719344,351.699497 C469.922848,350.935955 469.441059,350.380484 468.66246,350.504758 C468.66246,350.504758 468.060497,350.444919 467.50539,350.938198 C467.272758,351.144919 466.62035,351.549356 466.62035,351.549356 C465.788008,352.090124 465.810205,352.931307 466.669752,353.428089 L468.913067,354.72463 L469.719344,351.699497 L469.719344,351.699497 Z" class="cat-7" fill="#DFA461"></path>
						<path d="M469.478387,351.816307 C469.586679,351.259195 469.313315,351.086754 468.86292,351.43647 C468.86292,351.43647 468.207826,351.938403 468.021635,352.104066 C467.758585,352.338115 467.387177,352.664282 467.387177,352.664282 C466.962947,353.037744 467.008191,353.575294 467.49941,353.871679 L468.913067,354.72463 L469.478387,351.816307 L469.478387,351.816307 Z" class="cat-8" fill="#3D0044"></path>
						<ellipse class="cat-9" fill="#FFFFFF" opacity="0.5" cx="464.179487" cy="358.25641" rx="2.87179487" ry="1.79487179"></ellipse>
						<path d="M463.447834,367.040817 C463.447834,367.040817 463.731262,367.779173 464.076928,368.679664 L464.155197,368.88356 C464.502636,369.78867 464.048831,370.804723 463.147904,371.150556 L463.069364,371.180705 C462.165612,371.527623 461.129274,371.077206 460.761605,370.191454 L460.178383,368.786412" class="cat-10" stroke="#3D0044" fill="#FAA748"></path>
						<path d="M465.73132,367.040817 C465.73132,367.040817 465.447892,367.779173 465.102226,368.679664 L465.023957,368.88356 C464.676518,369.78867 465.130323,370.804723 466.03125,371.150556 L466.10979,371.180705 C467.013542,371.527623 468.04988,371.077206 468.417549,370.191454 L469.000771,368.786412" class="cat-11" stroke="#3D0044" fill="#FAA748"></path>
						<ellipse class="cat-12" fill="#3D0044" cx="462.025641" cy="355.74359" rx="0.717948718" ry="0.717948718"></ellipse>
						<ellipse class="cat-13" fill="#3D0044" cx="466.333333" cy="355.74359" rx="0.717948718" ry="0.717948718"></ellipse>
						<ellipse class="cat-14" fill="#3D0044" cx="464.179487" cy="357.179487" rx="0.717948718" ry="0.717948718"></ellipse>
					</g>
				</g>
				<g class="w2">
					<g clip-path="url(#w2-clip)">
						<rect class="w2-bg" fill="#BDE397" x="382" y="306" width="38" height="60"></rect>
						<rect class="w2-blind" fill="#A0857A" x="382" y="306" width="38" height="60"
							u init-with="p:[tro:center top]"
							when-blinds="a:[hiw-shade-bounceUp:2000:linear:0:i:a]"></rect>
					</g>
					<rect class="w2-shadow-btm" fill="#5B4F4B" x="381" y="376" width="40" height="5"></rect>
					<rect class="w2-ledge" fill="#C56C76" x="379" y="366" width="44" height="10"></rect>
					<rect class="w2-shadow-top" fill="#5B4F4B" x="387" y="313" width="28" height="7"></rect>
					<rect class="w2-shadow-mid" fill="#5B4F4B" x="387" y="342" width="28" height="7"></rect>
					<path d="M382,306 L420,306 L420,366 L382,366 L382,306 L382,306 L382,306 L382,306 Z M388,313 L414,313 L414,337 L388,337 L388,313 L388,313 L388,313 L388,313 Z M388,342 L414,342 L414,366 L388,366 L388,342 L388,342 L388,342 L388,342 Z" class="w2-casing" fill="#FFFFFF"></path>
				</g>
				<g class="w1">
					<g clip-path="url(#w1-clip)">
						<rect class="w1-bg" fill="#BDE397" x="329" y="306" width="38" height="60"></rect>
						<rect class="w1-blind" fill="#A0857A" x="329" y="306" width="38" height="60"
						u init-with="p:[tro:center top]"
						when-blinds="a:[hiw-shade-bounceUp:2000:linear:100:i:a]"></rect>
								<!-- u init-with="p:[tro:center top]"
								on-init="a:[hiw-shade-bounceUp:2000:linear:0:i:a]" -->
					</g>
					<rect class="w1-shadow-btm" fill="#5B4F4B" x="328" y="376" width="40" height="5"></rect>
					<rect class="w1-ledge" fill="#C56C76" x="326" y="366" width="44" height="10"></rect>
					<rect class="w1-shadow-top" fill="#5B4F4B" x="334" y="313" width="28" height="7"></rect>
					<rect class="w1-shadow-mid" fill="#5B4F4B" x="334" y="342" width="28" height="7"></rect>
					<path d="M329,306 L367,306 L367,366 L329,366 L329,306 L329,306 L329,306 L329,306 Z M335,313 L361,313 L361,337 L335,337 L335,313 L335,313 L335,313 L335,313 Z M335,342 L361,342 L361,366 L335,366 L335,342 L335,342 L335,342 L335,342 Z" class="w1-casing" fill="#FFFFFF"></path>
				</g>
				<g class="door">
					<rect class="door-casing" fill="#FFFFFF" x="324" y="444" width="50" height="58"></rect>
					<rect class="door-light" fill="#40484B" x="329" y="444" width="40" height="58"></rect>
					<path d="M329,444 L369,444 L369,502 L329,502 L329,444 L329,444 L329,444 L329,444 Z M337,444 L361,444 L361,450 L337,450 L337,444 L337,444 L337,444 L337,444 Z M337,457 L361,457 L361,477 L337,477 L337,457 L337,457 L337,457 L337,457 Z" class="door-front" fill="#A0857A"></path>
					<rect class="door-shade" fill="#FFFFFF" x="321" y="399" width="56" height="36"></rect>
					<rect class="door-shade-edge" fill="#C56C76" x="321" y="435" width="56" height="9"></rect>
				</g>
			</g>
			<g class="mailbox">
				<rect class="mbox-store" fill="#C56C76" x="272" y="454" width="28" height="15" rx="2"></rect>
				<rect class="mbox-hatch" fill="#A0857A" x="272" y="454" width="12" height="15" rx="2"></rect>
				<rect class="mbox-stand" fill="#FFFFFF" x="283" y="472" width="6" height="31"></rect>
			</g>
			<g class="pie">
				<polygon class="pie-1" fill="#B8B8B8" points="446 463 462 463 461 467 447 467"></polygon>
				<path d="M454,465 C458.970563,465 463,464.104569 463,463 C463,461.895431 458.970563,460 454,460 C449.029437,460 445,461.895431 445,463 C445,464.104569 449.029437,465 454,465 Z" class="pie-2" fill="#FFCC82"></path>
				<path d="M450.297852,461 L452.115,460.436037" class="pie-3" stroke="#B27455"></path>
				<path d="M458.115,461 L456.297852,460.436037" class="pie-4" stroke="#B27455"></path>
				<path d="M454.115,462.219958 L454.115,461.216079" class="pie-5" stroke="#B27455"></path>
				<path d="M448.5,450.5 C448.698676,449.593733 451.740234,451.669922 449.5,453.938477 C447.259766,456.207031 448.179783,460.407545 448.5,458.5" class="pie-6" stroke="#DAE3E6"></path>
				<path d="M458.859068,450.5 C458.859068,450.5 455.618833,451.669922 457.859068,453.938477 C460.099302,456.207031 459.179285,460.407545 458.859068,458.5" class="pie-7" stroke="#DAE3E6"></path>
				<path d="M453.859068,457.815873 C453.660391,458.513826 450.618833,456.91487 452.859068,455.167764 C455.099302,453.420659 454.179285,450.185674 453.859068,451.654751" class="pie-8" stroke="#DAE3E6"></path>
			</g>
		</g>
	</svg>
	<svg class="absolute top-0 left-0 full-xy" width="800px" height="600px" viewBox="0 0 800 600" style="background: #092D4B;"
		u
		init-with="p:[op:1]"
		on-init="a:[opacity:1:0:3000:easeInOutSine:0:1:f]:delay-6000 | s:[day-transition:public, foliage:depth(>1):1500, branch:public:1750]">
	    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
	        <g class="blue-tree">
	            <rect class="b-canopy" fill="#AADFFF" x="221" y="364" width="43" height="86" rx="21.5"
					init-after="canopy"
					init-with="p:[transform:scaleY(0), tro:center bottom]"
					on-init="a:[scaleY:0:1:300:easeOutBounce:0:1:f]"></rect>
	            <path d="M241.5,499.514151 L241.5,397.485849 L241.5,499.514151 Z" class="b-tree-trunk-1" stroke="#0B3B5E" stroke-width="5"
					init-after="foliage"
					init-with="p:[transform:scale(0), tro:center bottom]"
					on-init="a:[scaleY:0:1:800:easeInOutExpo:0:1:f]"></path>
	            <path d="M241.46694,432.899271 L232.618869,424.0512 L241.46694,432.899271 Z" class="b-tree-trunk-2" stroke="#0B3B5E" stroke-width="5"
					init-after="branch"
					init-with="p:[transform:scale(0), tro:left bottom]"
					on-init="a:[scaleY:0:1:800:easeInOutExpo:0:1:f]"></path>
	            <path d="M241.618869,412.899271 L250.46694,404.0512 L241.618869,412.899271 Z" class="b-tree-trunk-3" stroke="#0B3B5E" stroke-width="5"
					init-after="branch"
					init-with="p:[transform:scale(0), tro:right bottom]"
					on-init="a:[scaleY:0:1:800:easeInOutExpo:200:1:f]"></path>
	        </g>
	        <g class="purple-tree">
				<rect class="p-tree-canopy" fill="#8163AC" x="538" y="364" width="43" height="86" rx="21.5"
						u
						init-with="p:[transform:scaleY(0), tro:center bottom]"
						when-canopy="a:[scaleY:0:1:300:easeOutBounce:0:1:f]"></rect>
	            <polygon class="p-tree-trunk-1" stroke="#0B3B5E" stroke-width="5" points="559.5 499.514151 559.5 397.485849"
					u
					init-with="p:[transform:scale(0), tro:center bottom]"
					when-foliage="a:[scaleY:0:1:800:easeInOutExpo:0:1:f]"></polygon>
	            <polygon class="p-tree-trunk-2" stroke="#0B3B5E" stroke-width="5" points="559.53306 432.899271 568.381131 424.0512"
					u
					init-with="p:[transform:scale(0), tro:left bottom]"
					when-branch="a:[scaleY:0:1:800:easeInOutExpo:0:1:f]"></polygon>
	            <polygon class="p-tree-trunk-3" stroke="#0B3B5E" stroke-width="5" points="559.381131 412.899271 550.53306 404.0512"
					u
					init-with="p:[transform:scale(0), tro:right bottom]"
					when-branch="a:[scaleY:0:1:800:easeInOutExpo:200:1:f] | s:[canopy:public:2300]"></polygon>
	        </g>
	        <g class="purple-bushes">
	            <path d="M298.79867,504 C297.256668,481.09864 278.191583,463 254.899335,463 C231.607087,463 212.542002,481.09864 211,504 L298.79867,504 L298.79867,504 Z" class="prpl-bush-big" fill="#C3B6D6"
					u
					init-with="p:[transform:translateX(150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:150px:0px:800:easeOutCirc:0:1:f]"></path>
	            <path d="M202.404326,504 C201.76496,494.504314 193.859925,487 184.202163,487 C174.544402,487 166.639367,494.504314 166,504 L202.404326,504 L202.404326,504 Z" class="prpl-bush-solo" fill="#C3B6D6"
					u
					init-with="p:[transform:translateX(150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:150px:0px:1100:easeOutCirc:0:1:f]"></path>
	            <path d="M265.404326,504 C264.754498,493.945744 256.720115,486 246.904326,486 C237.088538,486 229.054155,493.945744 228.404326,504 L265.404326,504 L265.404326,504 Z" class="prpl-bush-small" fill="#8163AC"
					u
					init-with="p:[transform:translateX(150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:150px:0px:900:easeOutCirc:0:1:f]"></path>
	            <path d="M321,504 C319.953054,487.801477 307.008771,475 291.194444,475 C275.380118,475 262.435835,487.801477 261.388889,504 L321,504 L321,504 Z" class="prpl-bush-medium" fill="#8163AC"
					u
					init-with="p:[transform:translateX(150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:150px:0px:800:easeOutCirc:0:1:f]"></path>
	        </g>
	        <g class="blue-bushes">
	            <path d="M501.20133,504 C502.743332,481.09864 521.808417,463 545.100665,463 C568.392913,463 587.457998,481.09864 589,504 L501.20133,504 L501.20133,504 L501.20133,504 Z" class="blue-bush-big" fill="#2E5573"
					u
					init-with="p:[transform:translateX(-150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:-150px:0px:800:easeOutCirc:0:1:f]"></path>
	            <path d="M597.595674,504 C598.23504,494.504314 606.140075,487 615.797837,487 C625.455598,487 633.360633,494.504314 634,504 L597.595674,504 L597.595674,504 L597.595674,504 Z" class="blue-bush-solo" fill="#2E5573"
					u
					init-with="p:[transform:translateX(-150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:-150px:0px:1100:easeOutCirc:0:1:f]"></path>
	            <path d="M529.595674,504 C530.245502,493.945744 538.279885,486 548.095674,486 C557.911462,486 565.945845,493.945744 566.595674,504 L529.595674,504 L529.595674,504 L529.595674,504 Z" class="blue-bush-small" fill="#4E6D84"
					u
					init-with="p:[transform:translateX(-150px), op:0]"
					when-foliage="a:[opacity:0:1:500:linear:0:1:f, translateX:-150px:0px:900:easeOutCirc:0:1:f]"></path>
	            <path d="M474,504 C475.046946,487.801477 487.991229,475 503.805556,475 C519.619882,475 532.564165,487.801477 533.611111,504 L474,504 L474,504 L474,504 Z" class="blue-bush-medium" fill="#4E6D84"
					u
					init-with="p:[transform:translateX(-150px), op:0]"
					on-init="a:[opacity:0:1:500:linear:0:1:f, translateX:-150px:0px:800:easeOutCirc:0:1:f]"></path>
	        </g>
			<g class="sky" u on-init="s:[night-sky:>children:1100, hover-sky:>children:1200]">
				<g class="stars">
					<circle class="star-1" fill="#FFFFFF" cx="255.5" cy="258.5" r="2.5"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:2000:easeInOutQuad:500:i:a]"></circle>
					<circle class="star-2" fill="#FFFFFF" cx="272.5" cy="137.5" r="2.5"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:500:i:a]"></circle>
					<circle class="star-3" fill="#C5E9FE" cx="462.5" cy="129.5" r="2.5"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:500:i:a]"></circle>
					<circle class="star-4" fill="#C5E9FE" cx="577.5" cy="231.5" r="2.5"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:500:i:a]"></circle>
					<circle class="star-5" fill="#C5E9FE" cx="567.5" cy="319.5" r="1.5"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:500:i:a]"></circle>
					<circle class="star-6" fill="#FFFFFF" cx="266" cy="306" r="2"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:0:i:a]"></circle>
					<path d="M328.839652,146.767195 C327.82364,146.343485 327.830745,145.653552 328.839652,145.232805 L332.923049,143.529896 C333.939061,143.106186 335.109149,141.931956 335.529896,140.923049 L337.232805,136.839652 C337.656515,135.82364 338.346448,135.830745 338.767195,136.839652 L340.470104,140.923049 C340.893814,141.939061 342.068044,143.109149 343.076951,143.529896 L347.160348,145.232805 C348.17636,145.656515 348.169255,146.346448 347.160348,146.767195 L343.076951,148.470104 C342.060939,148.893814 340.890851,150.068044 340.470104,151.076951 L338.767195,155.160348 C338.343485,156.17636 337.653552,156.169255 337.232805,155.160348 L335.529896,151.076951 C335.106186,150.060939 333.931956,148.890851 332.923049,148.470104 L328.839652,146.767195 Z" class="star-7" fill="#FFFFFF"
						u
						init-with="p:[transform:scale(0) rotate(0deg), tro:center center]"
						when-night-sky="a:[scaleIn-subtle:2000:linear:500:i:a]"></path>
					<path d="M403.849736,117.2714 C402.828155,116.845368 402.823135,116.156726 403.849736,115.7286 L404.031615,115.652751 C405.053196,115.226718 406.224624,114.058216 406.652751,113.031615 L406.7286,112.849736 C407.154632,111.828155 407.843274,111.823135 408.2714,112.849736 L408.347249,113.031615 C408.773282,114.053196 409.941784,115.224624 410.968385,115.652751 L411.150264,115.7286 C412.171845,116.154632 412.176865,116.843274 411.150264,117.2714 L410.968385,117.347249 C409.946804,117.773282 408.775376,118.941784 408.347249,119.968385 L408.2714,120.150264 C407.845368,121.171845 407.156726,121.176865 406.7286,120.150264 L406.652751,119.968385 C406.226718,118.946804 405.058216,117.775376 404.031615,117.347249 L403.849736,117.2714 Z" class="star-8" fill="#FFFFFF"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1100:easeInOutQuad:750:i:a]"></path>
					<path d="M432.839299,213.267047 C431.823482,212.843419 431.820073,212.158003 432.839299,211.732953 L433.747752,211.354098 C434.763569,210.930469 435.929048,209.766978 436.354098,208.747752 L436.732953,207.839299 C437.156581,206.823482 437.841997,206.820073 438.267047,207.839299 L438.645902,208.747752 C439.069531,209.763569 440.233022,210.929048 441.252248,211.354098 L442.160701,211.732953 C443.176518,212.156581 443.179927,212.841997 442.160701,213.267047 L441.252248,213.645902 C440.236431,214.069531 439.070952,215.233022 438.645902,216.252248 L438.267047,217.160701 C437.843419,218.176518 437.158003,218.179927 436.732953,217.160701 L436.354098,216.252248 C435.930469,215.236431 434.766978,214.070952 433.747752,213.645902 L432.839299,213.267047 Z" class="star-9" fill="#FFFFFF"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1600:easeInOutQuad:1250:i:a]"></path>
					<path d="M278.839437,178.767105 C277.823544,178.343445 277.826828,177.655186 278.839437,177.232895 L280.100464,176.707006 C281.116357,176.283345 282.284715,175.113072 282.707006,174.100464 L283.232895,172.839437 C283.656555,171.823544 284.344814,171.826828 284.767105,172.839437 L285.292994,174.100464 C285.716655,175.116357 286.886928,176.284715 287.899536,176.707006 L289.160563,177.232895 C290.176456,177.656555 290.173172,178.344814 289.160563,178.767105 L287.899536,179.292994 C286.883643,179.716655 285.715285,180.886928 285.292994,181.899536 L284.767105,183.160563 C284.343445,184.176456 283.655186,184.173172 283.232895,183.160563 L282.707006,181.899536 C282.283345,180.883643 281.113072,179.715285 280.100464,179.292994 L278.839437,178.767105 Z" class="star-10" fill="#C5E9FE"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1100:easeInOutQuad:500:i:a]"></path>
					<path d="M530.851147,280.271989 C529.828787,279.845631 529.826992,279.155117 530.851147,278.728011 L532.441603,278.064739 C533.463964,277.638382 534.637633,276.465758 535.064739,275.441603 L535.728011,273.851147 C536.154369,272.828787 536.844883,272.826992 537.271989,273.851147 L537.935261,275.441603 C538.361618,276.463964 539.534242,277.637633 540.558397,278.064739 L542.148853,278.728011 C543.171213,279.154369 543.173008,279.844883 542.148853,280.271989 L540.558397,280.935261 C539.536036,281.361618 538.362367,282.534242 537.935261,283.558397 L537.271989,285.148853 C536.845631,286.171213 536.155117,286.173008 535.728011,285.148853 L535.064739,283.558397 C534.638382,282.536036 533.465758,281.362367 532.441603,280.935261 L530.851147,280.271989 Z" class="star-11" fill="#FFFFFF"
						u
						init-with="p:[op:0, tro:center center]"
						when-night-sky="a:[scaleIn-subtle:1800:linear:500:i:a]"></path>
				</g>
				<g u
					init-with="p:[transform-origin:right top, transform:rotate(90deg) scale(0)]"
					when-night-sky="a:[rotate:90deg:0deg:500:easeOutQuart:0:1:f, scale:0:1:300:easeOutQuart:0:1:f]">
					<path d="M560.15882,201.030391 C552.128267,209.626465 540.691661,215 528,215 C503.699471,215 484,195.300529 484,171 C484,146.699471 503.699471,127 528,127 C541.337224,127 553.288467,132.934088 561.357524,142.306058 C558.554356,141.45669 555.580511,141 552.5,141 C535.655315,141 522,154.655315 522,171.5 C522,188.344685 535.655315,202 552.5,202 C555.145091,202 557.711539,201.663289 560.15882,201.030391 L560.15882,201.030391 Z" class="moon" fill="#FFFFFF"></path>
					<!-- on-exit="a:[translateY:0px:-800px:400:easeInOutBack:0:1:f]:delay-5000" -->
				</g>
				<g class="big-cloud"
					u
					when-hover-sky="a:[translateY:-3px:3px:900:easeInOutSine:200:i:a]">
					<path d="M376.178959,232 C376.715756,229.754184 377,227.410286 377,225 C377,208.431458 363.568542,195 347,195 C330.431458,195 317,208.431458 317,225 C317,227.410286 317.284244,229.754184 317.821041,232 L376.178959,232 L376.178959,232 Z" class="b-cloud-1" fill="#BCE5FF"
						u
						init-with="p:[transform:scale(0), tro:center bottom]"
						when-night-sky="a:[scale:0:1:200:easeOutSine:0:1:f]"></path>
					<path d="M321.243895,232 C321.737482,230.08228 322,228.071797 322,226 C322,212.745166 311.254834,202 298,202 C284.745166,202 274,212.745166 274,226 C274,228.071797 274.262518,230.08228 274.756105,232 L321.243895,232 L321.243895,232 Z" class="b-cloud-2" fill="#BCE5FF"
						u
						init-with="p:[transform:scale(0), tro:center bottom]"
						when-night-sky="a:[scale:0:1:200:easeOutSine:0:1:f]"></path>
					<path d="M281.975766,232 C282.639075,230.123326 283,228.103813 283,226 C283,216.058875 274.941125,208 265,208 C255.058875,208 247,216.058875 247,226 C247,228.103813 247.360925,230.123326 248.024234,232 L281.975766,232 L281.975766,232 Z" class="b-cloud-3" fill="#BCE5FF"
						u
						init-with="p:[transform:scale(0), tro:center bottom]"
						when-night-sky="a:[scale:0:1:200:easeOutSine:0:1:f]"></path>
				</g>
				<g class="small-cloud"
					u
					when-hover-sky="a:[translateY:3px:-3px:900:easeInOutSine:200:i:a]">
					<path d="M378.756105,178 C378.262518,176.08228 378,174.071797 378,172 C378,158.745166 388.745166,148 402,148 C415.254834,148 426,158.745166 426,172 C426,174.071797 425.737482,176.08228 425.243895,178 L378.756105,178 L378.756105,178 Z" class="s-cloud-1" fill="#BCE5FF"
						u
						init-with="p:[transform:scale(0), tro:center bottom]"
						when-night-sky="a:[scale:0:1:200:easeOutSine:0:1:f]"></path>
					<path d="M418.024234,178 C417.360925,176.123326 417,174.103813 417,172 C417,162.058875 425.058875,154 435,154 C444.941125,154 453,162.058875 453,172 C453,174.103813 452.639075,176.123326 451.975766,178 L418.024234,178 L418.024234,178 Z" class="s-cloud-2" fill="#BCE5FF"
						u
						init-with="p:[transform:scale(0), tro:center bottom]"
						when-night-sky="a:[scale:0:1:200:easeOutSine:0:1:f]"></path>
				</g>
	        </g>
	        <g class="house">
	            <rect class="house-foundation" fill="#AADFFF" x="307" y="285" width="187" height="204"
					u init-with="p:[transform:scaleY(0), transform-origin:center bottom]"
					on-init="a:[scaleY:0:1:1000:easeOutBounce:0:1:f] | s:[build-house:siblings:500]"></rect>
	            <rect class="house-casing" fill="#FFFFFF" x="301" y="485" width="199" height="19"
					u
					init-with="p:[tro:center bottom, transform:scaleY(0)]"
					when-build-house="a:[scaleY:0:1:500:easeOutBounce:0:1:f]"></rect>
	            <rect class="roof-edge" fill="#FFFFFF" x="299" y="258" width="203" height="19"
					u
					init-with="p:[tro:center bottom, transform:scaleY(0)]"
					when-build-house="a:[scaleY:0:1:750:easeOutQuad:0:1:f]"></rect>
	            <rect class="roof-blue" fill="#AADFFF" x="299" y="258" width="203" height="12"
					u
					init-with="p:[tro:center bottom, transform:scaleY(0)]"
					when-build-house="a:[scaleY:0:1:750:easeOutQuad:0:1:f]"></rect>
	            <rect class="roof-top" fill="#FFFFFF" x="296" y="258" width="209" height="7"
					u
					init-with="p:[tro:center bottom, transform:scaleY(0)]"
					when-build-house="a:[scaleY:0:1:750:easeOutBounce:0:1:f]"></rect>
				<g class="windows"
					u
					on-init="s:[w1:children]">
					<g class="w5"
						u
						init-with="p:[opacity:0, tro:20% 80%]"
						when-w1="a:[zoom-enter:800:linear:400:1:f]">
						<g clip-path="url(#w5-clip)">
							<rect class="w5-bg" fill="#FEF1AE" x="435" y="407" width="38" height="60"></rect>
							<rect class="w5-blind" fill="#2E5573" x="435" y="407" width="38" height="60"></rect>
								<!-- init-after="w4"
								on-init="a:[fill:#2E5573:#FEF1AE:5000:easeInOutSine:0:i:a]" -->
						</g>
		            	<rect class="w5-shadow-btm" fill="#0B3B5E" x="434" y="477" width="40" height="5"></rect>
			            <rect class="w5-ledge" fill="#8C70B3" x="432" y="467" width="44" height="10"></rect>
			            <rect class="w5-shadow-top" fill="#0B3B5E" x="440" y="414" width="28" height="7"></rect>
			            <rect class="w5-shadow-mid" fill="#0B3B5E" x="440" y="443" width="28" height="7"></rect>
			            <path d="M435,407 L473,407 L473,467 L435,467 L435,407 L435,407 Z M441,414 L467,414 L467,438 L441,438 L441,414 L441,414 Z M441,443 L467,443 L467,467 L441,467 L441,443 L441,443 Z" class="w5-casing" fill="#FFFFFF"></path>
					</g>
					<g class="w4"
						u
						init-with="p:[op:0, tro:20% 80%]"
						when-w1="a:[zoom-enter:800:linear:300:1:f]">
			            <rect class="w4-shadow-btm" fill="#0B3B5E" x="381" y="477" width="40" height="5"></rect>
			            <rect class="w4-ledge" fill="#8C70B3" x="379" y="467" width="44" height="10"></rect>
			            <rect class="w4-blind" fill="#2E5573" x="382" y="407" width="38" height="60"></rect>
			            <rect class="w4-shadow-top" fill="#0B3B5E" x="387" y="414" width="28" height="7"></rect>
			            <rect class="w4-shadow-mid" fill="#0B3B5E" x="387" y="443" width="28" height="7"></rect>
			            <path d="M382,407 L420,407 L420,467 L382,467 L382,407 L382,407 Z M388,414 L414,414 L414,438 L388,438 L388,414 L388,414 Z M388,443 L414,443 L414,467 L388,467 L388,443 L388,443 Z" class="w4-casing" fill="#FFFFFF"></path>
					</g>
					<g class="w3"
						u
						init-with="p:[op:0, tro:20% 80%]"
						when-w1="a:[zoom-enter:1000:linear:200:1:f]">
			            <rect class="w3-shadow-btm" fill="#0B3B5E" x="434" y="376" width="40" height="5"></rect>
			            <rect class="w3-ledge" fill="#8C70B3" x="432" y="366" width="44" height="10"></rect>
			            <rect class="w3-blind" fill="#2E5573" x="435" y="306" width="38" height="60"></rect>
			            <rect class="w3-shadow-top" fill="#0B3B5E" x="440" y="313" width="28" height="7"></rect>
			            <rect class="w3-shadow-mid" fill="#0B3B5E" x="440" y="342" width="28" height="7"></rect>
			            <path d="M435,306 L473,306 L473,366 L435,366 L435,306 L435,306 Z M441,313 L467,313 L467,337 L441,337 L441,313 L441,313 Z M441,342 L467,342 L467,366 L441,366 L441,342 L441,342 Z" class="w3-casing" fill="#FFFFFF"></path>
					</g>
					<g class="w2"
						u
						init-with="p:[op:0, tro:20% 80%]"
						when-w1="a:[zoom-enter:1000:linear:100:1:f]">
						<g clip-path="url(#w2-clip)">
							<rect class="w2-bg" fill="#FEF1AE" x="382" y="306" width="38" height="60"></rect>
				            <rect class="w2-blind" fill="#2E5573" x="382" y="306" width="38" height="60"></rect>
							<!--
								init-after="w1"
								on-init="a:[fill:#2E5573:#FEF1AE:3000:easeInOutSine:0:i:a]" -->
						</g>
			            <rect class="w2-shadow-btm" fill="#0B3B5E" x="381" y="376" width="40" height="5"></rect>
			            <rect class="w2-ledge" fill="#8C70B3" x="379" y="366" width="44" height="10"></rect>
			            <rect class="w2-shadow-top" fill="#0B3B5E" x="387" y="313" width="28" height="7"></rect>
			            <rect class="w2-shadow-mid" fill="#0B3B5E" x="387" y="342" width="28" height="7"></rect>
			            <path d="M382,306 L420,306 L420,366 L382,366 L382,306 L382,306 Z M388,313 L414,313 L414,337 L388,337 L388,313 L388,313 Z M388,342 L414,342 L414,366 L388,366 L388,342 L388,342 Z" class="w2-casing" fill="#FFFFFF"></path>
					</g>
					<g class="w1"
						u
						init-with="p:[op:0, tro:20% 80%]"
						when-w1="a:[zoom-enter:1000:linear:0:1:f]">
			            <rect class="w1-shadow-btm" fill="#0B3B5E" x="328" y="376" width="40" height="5"></rect>
			            <rect class="w1-ledge" fill="#8C70B3" x="326" y="366" width="44" height="10"></rect>
			            <rect class="w1-blind" fill="#2E5573" x="329" y="306" width="38" height="60"></rect>
			            <rect class="w1-shadow-top" fill="#0B3B5E" x="334" y="313" width="28" height="7"></rect>
			            <rect class="w1-shadow-mid" fill="#0B3B5E" x="334" y="342" width="28" height="7"></rect>
			            <path d="M329,306 L367,306 L367,366 L329,366 L329,306 L329,306 Z M335,313 L361,313 L361,337 L335,337 L335,313 L335,313 Z M335,342 L361,342 L361,366 L335,366 L335,342 L335,342 Z" class="w-casing" fill="#FFFFFF"></path>
					</g>
				</g>
				<g class="door" u on-init="s:[door:children]">
		            <rect class="door-casing" fill="#FFFFFF" x="324" y="444" width="50" height="58"
						u
						init-with="p:[tro:center top, transform:scaleY(0)]"
						when-door="a:[scaleY:0:1:200:easeOutSine:300:1:f]"></rect>
		            <rect class="door-light" fill="#FEF1AE" x="329" y="444" width="40" height="58"
						u
						init-with="p:[tro:center top, transform:scaleY(0)]"
						when-door="a:[scaleY:0:1:200:easeOutSine:250:1:f]"></rect>
		            <path d="M329,444 L369,444 L369,502 L329,502 L329,444 L329,444 Z M337,444 L361,444 L361,450 L337,450 L337,444 L337,444 Z M337,457 L361,457 L361,477 L337,477 L337,457 L337,457 Z" class="door-front" fill="#2E5573"
						u
						init-with="p:[tro:center top, transform:scaleY(0)]"
						when-door="a:[scaleY:0:1:200:easeOutSine:200:1:f]"></path>
		            <rect class="door-shade" fill="#FFFFFF" x="321" y="399" width="56" height="36"
						u
						init-with="p:[op:0, tro:center top]"
						when-door="a:[bounceIn-subtle:1000:linear:0:1:f]"></rect>
						<!--on-init="a:[bounceIn-subtle:500:linear:0:1:f]:delay-500" -->
		            <rect class="door-shade-edge" fill="#8A70B3" x="321" y="435" width="56" height="9"
						u
						init-with="p:[tro:center top, transform:scaleY(0)]"
						when-door="a:[scaleY:0:1:200:easeOutSine:0:1:f]"></rect>
					</g>
				</g>
	        </g>
	        <g class="mailbox" u on-init="s:[mailbox:children]">
	            <rect class="mbox-store" fill="#8C70B3" x="272" y="454" width="28" height="15" rx="2"
					u init-with="p:[tro:left center, transform:scaleX(0)]"
					when-mailbox="a:[scaleX:0:1:300:easeOutBack:250:1:f]"></rect>
	            <rect class="mbox-hatch" fill="#B5E3FE" x="272" y="454" width="12" height="15" rx="2"
					u init-with="p:[tro:left center, transform:scaleX(0)]"
					when-mailbox="a:[scaleX:0:1:300:easeOutBack:200:1:f]"></rect>
	            <rect class="mbox-stand" fill="#FFFFFF" x="283" y="473" width="6" height="31"
					u init-with="p:[tro:center bottom, transform:scaleY(0)]"
					when-mailbox="a:[scaleY:0:1:300:easeOutBack:0:1:f]"></rect>
	        </g>
	    </g>
	</svg>
</div>
