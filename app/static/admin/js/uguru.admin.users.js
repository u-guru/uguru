...
function getUsers() {
	$.ajax({
		...
		success: function(data){
			var users = data;
			$('#user-table').html("");
			var header_row_attr = ['id','Name', 'Email', 'Majors', 'Guru Courses', 'Student Courses', 'University'];
			var user_row_header = computeTableHeaderUser(header_row_attr)
			$('#user-table').append(user_row_header);
			for(i = 0; i < users.length; i++){
				var user = users[i];
				var user_attr_arr = [user.id, user.name, user.email, processListObjToString(user.majors, 'name'),
				processListObjToString(user.guru_courses, 'short_name'), processListObjToString(user.student_courses, 'short_name'),
				user.university.title]
				var user_row_string = computeTableRowUser(user_attr_arr, i, user.id);
...
function computeTableRowUser(attr_arr, table_index, user_id) {
	var result_str = ''
	...
	for (j = 0; j < attr_arr.length; j++) {
		if (j === 0) {
			...
			result_str += '<td class="sorting_1"> <a style="text-decoration:underline;" href="/admin/user/' + user_id + '">' + attr_arr[j] + ' </td>';
