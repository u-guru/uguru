...
var stripeCreditModalParentsResponseHandler = function(status, response) {
    ...
    $('.student-request-accept-btn-credit').click(function(){
        if (!request_a_guru_clicked) {
            ...
            if (!$('#student-signup-description').val() || !$('#student-signup-location').val() || !$('#student-signup-availability').val() || !$('#student-signup-skill').val()) {
...
$('#submit-bank-account-info').click(function() {
    if (!$('#full-legal-name').val() || !$('#bank-account-num').val() || !$('#bank-routing-num').val()) {
            ...
            Stripe.bankAccount.createToken({
                ...
                if (response.error) {
                    ...
                    $('#bank-account-alert').show();
                        ...
                        var token = response.id;
                        var data = {'token':token, 'bank':response['bank_account']['bank_name'], 'legal-name':$('#full-legal-name').val(), last4:response['bank_account']['last4']};
                        $.ajax({
                            type: "POST",
                            contentType: 'application/json;charset=UTF-8',
                            url: '/add-bank/' ,
