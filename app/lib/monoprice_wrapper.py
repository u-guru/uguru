...
cookie_file = open('cookies_from_this_file')
...
def fill_in_last_form_1():
	payload = {'step': 'step 1'}
	update_last_step = session.post('https://www.monoprice.com/checkout/updatelaststep',data=payload,cookies = cookies).text
...
def start_checkout():
	...
	start_checkout_url = session.post('https://www.monoprice.com/checkout/startcheckout', data=payload,cookies = cookies).text 
	soup = BeautifulSoup(start_checkout_url)
#start_checkout()

def visit_checkout_step_2():
	params = {'skip_qas':'no','use_profile_address':'1493978','cmd':'used'}
	vist_shipping_url = session.post('https://www.monoprice.com/checkout/checkoutstep2',data=params,cookies = cookies).text
...
def visit_checkout_step_3_only():
	...
	request_url_info = session.post('https://www.monoprice.com/checkout/checkoutstep3',data=payload,cookies = cookies).text
...
def pay_with_credit_card_info(credit_card_number,expire_month,expire_year,card_holder_name,card_security_four_digit_pin,phone_number):
	...
	finalization = session.post('https://www.monoprice.com/checkout/checkoutstep3b',data=payload,cookies = cookies).text
	response = BeautifulSoup(finalization)
	#print response
	print "Credit Card Info Inserted!"

# pay_with_credit_card_info('4815820084431326','09','2019','Samir Makhani','393','5107344513')


def preview_page():
	payload = {'cmd':'modify','use_profile_address':'1493978'}
	final_step = session.post('https://www.monoprice.com/checkout/checkoutstep4',data=payload,cookies = cookies).text
...
def purchase_item():
	...
	purchase_item = session.post('https://www.monoprice.com/checkout/checkoutregister',data = payload,cookies = cookies).text
