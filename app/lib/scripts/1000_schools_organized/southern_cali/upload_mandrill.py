...
with open('uc_southern_cali.json') as datafile:
        json_data_file = json.load(datafile)
        for email in json_data_file:
                dictionary = {}
                dictionary['email'] =  email['email'].replace('\n',' ')
                dictionary['first_name'] = email['first_name'].replace('\n',' ')
                dictionary['last_name'] = email['last_name'].replace('\n',' ')
        ...
        for x in range(1,len(huge_arr)):
                ...
                if (x % 999 == 0):
                        add_students_to_mailing_list('University of Southern California',second_array)
                        response = add_students_to_mailing_list('University of Southern California',second_array)
                        print response.text
