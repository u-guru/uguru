...
with open('yeshiva_school_data.json') as datafile:
        ...
        for email in json_data_file:
                ...
                dictionary['type'] = email['student_type']
        ...
        for x in range(1,len(huge_arr)):
                ...
                if (x % 999 == 0):
                        add_students_to_mailing_list('Yeshiva University',second_array)
                        response = add_students_to_mailing_list('Yeshiva University',second_array)
                        print response.text
