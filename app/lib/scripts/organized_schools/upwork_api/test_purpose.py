import tor_module, ast, json,string
from bs4 import BeautifulSoup

SHCHOOL_ARRAY = ['ucla','University of Vermont','uc berkeley']
errors_array = []
KEY_ARRAY = []	
OUTPUT_PATH = "wikipedia_university_9.json"
error_path = "wikipedia_error_9.json"
# Change this URL
...
#url = "http://en.wikipedia.org/w/api.php?action=query&prop=revisions%7Cinfo%7Cimages&titles=UCLA&rvprop=timestamp%7Cuser%7Ccomment%7Ccontent&inprop=url&redirects&iiprop=url&rvparse"
with open('school-data-sanitized.json') as output_file:
	data_info  = json.load(output_file)
	print len(data_info)
	start = 2520
	end = start + 280
	index = start - 1
	print "Proccessing from %d to %d" % (start,end)
	for info in data_info[start:end]:
		index += 1
		sanitized_name = info['sanitized_name']
		print "Proccess the %d %s" %(index, sanitized_name)
		capt = string.capwords(sanitized_name).replace('-',' ').replace('Of','of').replace('At','at').replace('The','the').replace('&','and').replace('And','and')
...
# Iterate through every page
		for page in dictionary:
			...
			# Get the URL for the 	current page
				if dictionary[page].get('fullurl'):
		   			page_url = dictionary[page].get('fullurl')
			# Construct the beautiful soup object for the current page
				...
				# Get the table on the right hand side of the page
					...
					# Get all the rows for the table
						...
						unique_value = {}
						college_name = soup.find('h1', attrs = {'id':'firstHeading'}).text

					except:
						### we are on the disambiguation page
						try:
							base_wiki_url = 'https://en.wikipedia.org'
							full_url = base_wiki_url + soup.select('#bodyContent #mw-content-text ul a')[0]['href']
							soup = BeautifulSoup(tor_module.get(full_url).text)
							table = soup.find('table', class_ = "infobox")
						...
						# Get all the rows for the table
							...
							print "ERROR IN WIKIPEDIA WHILE PARSING DISAMBIG WIKI PAGE", capt
							errors_array.append(capt)
							with open(error_path,'w') as outfile:
								json.dump (errors_array,outfile,indent = 4)
								print len(errors_array)
							continue
				...
				with open(error_path,'w') as outfile:
					json.dump(errors_array,outfile,indent = 4)
					print "ERRORS NUMBER" ,len(errors_array)
				continue
			...
			try:
							...
							if row.th.text == "Location":
								...
								if college_location:
									...
									for actual_cord in college_location:
										longitude_latitude['Coordinates'] = college_location.find('span', attrs = {'class':'geo-default'}).text.replace('\u00b0','')
										location.append(longitude_latitude)
									school['coords'] = location
										

									
							for info_college in rows:
								colllege_image_link_url = []
								image_url = info_college.findAll('img')
								for image_src in image_url:
										image_url = image_src['src']
										colllege_image_link_url.append(image_url)
										school['college_url'] = colllege_image_link_url		
							...
							if row.th.text == "Affiliations":
								...
								for college_affiliaiton in affiliaitons_all_tgs:
									try:
										...
										pass
								school['Affiliatons'] = affiliationsz
				...
				print "ERROR",capt
				raise						
			unique_value[college_name] = school	
			KEY_ARRAY.append(unique_value)
			with open(OUTPUT_PATH, 'w') as outfile:
				json.dump(KEY_ARRAY, outfile, indent = 4)