...
def get_school_information():
	...
	schools = soup.find('div', attrs = {'class': 'schools-list'}).select('li a')
	
	print len(schools)
	for school in schools:
		sch = {}

		sch['chegg-name'] = school['href'].split('/')[-1]
		

		formal_school_string = school.find('span', attrs = {'class': 'code'}).string.lower().replace('-', ',').replace('/', ',')
		clean_school_string = " ".join(formal_school_string.replace(',', ' ').split())
		sch['name'] = clean_school_string
		sch['official_name'] = formal_school_string
